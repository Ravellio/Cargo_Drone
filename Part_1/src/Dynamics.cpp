...
DroneCargoState::DroneCargoState(void) {
	...
	vec = { {"xD", 0}, {"yD", droneDefaultHeight}, {"thetaD", 0}, {"xDotD", 0}, {"yDotD", 0}, {"xC", 0}, {"yC", 0}, {"xDotC", 0}, {"yDotC", 0}, {"lRope", 0}, {"lDotRope", 0} };
}
...
State::svec DroneState::f(const svec& v, const std::vector<float>& u) {
	float drag = cDragDrone * sqrt(pow(v.at("xDot"), 2) + pow(v.at("yDot"), 2)); 
	float thetaDot = 0;
...
State::svec DroneCargoState::f(const svec& v, const std::vector<float>& u) {
	float dragDrone = cDragDrone * sqrt(pow(v.at("xDotD"), 2) + pow(v.at("yDotD"), 2));
	float dragCargo = cDragCargo * sqrt(pow(v.at("xDotC"), 2) + pow(v.at("yDotC"), 2));

	float lRope = sqrt(pow(v.at("xD") - v.at("xC"), 2) + pow(v.at("yD") - v.at("yC"), 2));
	...
	return {
		...
		{"xC", v.at("xDotC")},
		{"yC", v.at("yDotC")},
		{"xDotC", (1 / mC) * (-cDragCargo * v.at("xDotC") + fRopeX)},
		{"yDotC", (1 / mC) * (-cDragCargo * v.at("yDotC") + fRopeY) - g},
		{"lRope", lRope},
		{"lDotRope", lDotRope}
	};
}
